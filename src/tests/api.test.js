...
describe("Error handling in getWeather", () => {
  ...
  test("getWeather returns the error message when the city is not found", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      ...
      expect.any(Error)
    ); // assert that the console.error was called with the right arguments
  });
});
...
describe("generateDynamicResponse returns value check", () => {
      ...
      choices: [
        {
          message: {
            content:
              "With this weather in London, you can go for a walk in the park, have a picnic, or play outdoor sports.",
          },
        },
      ],
    });
  ...
  test("generateDynamicResponse returns null when the API call fails", async () => {
    ...
    const consoleSpy = jest
      .spyOn(console, "error")
      .mockImplementation(() => {}); // temperrarily disable console.error to avoid polluting jest's output
